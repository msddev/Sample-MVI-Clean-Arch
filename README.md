Android Clean Architecture in Rich and Morty is a sample project that presents a modern, approach to [Android](https://www.android.com/) application development using [Kotlin](https://kotlinlang.org/) and the latest tech stack.

The goal of the project is to demonstrate best practices, provide a set of guidelines, and present modern Android
application architecture that is modular, scalable, maintainable, and testable. This application may look simple, but it
has all of these small details that will set the rock-solid foundation of the more extensive app suitable for bigger teams and
long application lifecycle management.

An android app built using Kotlin that consumes [Rick and Morty API](https://rickandmortyapi.com) to display characters from the [TV Series](https://www.imdb.com/title/tt2861424/). It has been built following Clean Architecture Principle, Repository Pattern, MVI Architecture in the presentation layer as well as Jetpack components.
...
- Another principle of Clean Architecture is the [Dependency Inversion](https://en.wikipedia.org/wiki/Dependency_inversion_principle). This rule specifies that each circle can depend only on the nearest inward circle i.e.. low-level modules do not depend on high-level modules but the other way around.
...
### BuildSrc
The ```buildSrc``` layer is responsible for dependency management. It controls and manages all dependencies in one place with Kotlin.

### Common
The ```common``` layer is responsible for common properties. It contains the implementations of the resource files, themes, and components declared in the common layer.

- __component__: This is responsible for common view components that use the app.
...
### Libraries
The ```libraries``` layer is responsible for the base structure.
...
- __model__
  ...
  -__dto__: Defines dto of ui model, also performs data transformation between ```domain```, ```response``` and ```entity``` models.
...
### Presentation
The ```features``` layer contains components involved in showing information to the user. The main parts of this layer are the views(activity, compose) and ViewModels.
...
- [Accompanist](https://google.github.io/accompanist)
    ...
    - [Insets](https://google.github.io/accompanist/insets/) - Insets for Jetpack Compose takes a lot of the ideas that drove Insetter for views and applies them for use in composables.
    - [Placeholder](https://google.github.io/accompanist/placeholder/) - A library that provides a modifier for displaying the 'placeholder' UI while content is loading.
    - [Navigation](https://google.github.io/accompanist/navigation-material/) - A library that provides Compose Material support for Jetpack Navigation Compose. This features composable bottom sheet destinations.
...
```license
Copyright © 2022 - Masoud Khoshkam
